...
public class PullExtendLayout extends LinearLayout implements IPullToExtend {
    ...
    protected void onFinishInflate() {
        ...
        for (int i = 0; i < getChildCount(); i++) {
            ...
            if (child instanceof ExtendListHeader) {
                mHeaderLayout = (ExtendLayout) child;
            }
            if (child instanceof ExtendListFooter) {
                mFooterLayout = (ExtendLayout) child;
            }
...
//        mRefreshableView.setLayoutParams(layoutParams);
    ...
    private void refreshLoadingViewsSize() {
        ...
        ((ExtendListHeader) mHeaderLayout).setListHeight(mHeaderHeight);
    ...
    public final boolean onInterceptTouchEvent(MotionEvent event) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                // 位移差大于mTouchSlop，这是为了防止快速拖动引发刷新,仅拦截上下滑动事件

                if (((deltaY > 0 && (absDiff > mTouchSlop)) || Math.abs(getScrollYValue()) > 0) && Math.abs(deltaY) > Math.abs(deltaX)) {
                    mLastMotionY = event.getY();
...
//                        }
                ...
                //头部显示出来后,在头部下方左右滑动收起头部
                if (Math.abs(deltaX) > Math.abs(deltaY) && Math.abs(getScrollYValue()) > 0 && Math.abs(mLastMotionY) > mHeaderHeight) {
                    resetHeaderLayout();
